...
//14. Multiple Sum
function sumMultiples(n: number, divisor: number): string {
  let SumEvenNumber = 0;
  for (let i = 1; i <= n; i++) {
    if (i % divisor == 0) {
...
//1. Sum of Positives
function sumOfPositives(myArray: number[]): string {
  let sum = 0;
...
function findMax(bigger: number[]): string {
  let big = bigger[0];
...
//4. Longest word
function findLongestWord(longer: string[]): string {
  let long = longer[0];
...
//5. Count Properties
function countProperties(candidate: object): number {
...
function sumEvenNumber(arrNumber: number[]): string {
  let sumEvenArray = 0;
...
function differenceEvenOdd(minusEvenOdd: number[]): number {
  let minusEven = 0,
    minusOdd = 0;
...
// 9. Count Truthy
function countTruthy(truthyObject: Record<string, unknown>): number {
  let objectCount = 0;
...
function average(arrAverage: number[]): number {
  let sumAverage = 0;
...
//11. Linear Search
function linearSearch(arrLinear: number[], searchNumber: number): number {
...
function countOccurrences({
  ...
  let countArray: Record<string, number> = {};
...
function removeDuplicates({ arr }: { arr: number[] }): number[] {
...
//16. Most Frequent
function mostFrequent({ arrFrequent }: { arrFrequent: number[] }) {
  const objectFrequent: { [key: number]: number } = {};
  let maxFrequency = 0;
